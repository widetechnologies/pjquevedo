...
class ClassBuilder {
    ...
    private function Load() {
        ...
        if ($this->table_descriptor->getPrimaryKey() != '') {
            ...
            $buf .= "\t\t\$result = mysqli_query(\$link, \$query);\n\n";
            $buf .= "\t\tif(mysqli_num_rows(\$result) > 0) {\n";
            $buf .= "\t\t\twhile(\$row = mysqli_fetch_assoc(\$result) ){\n";
            ...
            $buf .= "\t\tmysqli_query(\$link, \$query);\n\n";
            $buf .= "\t\treturn mysqli_affected_rows(\$link);\n";
            $buf .= "\t}\n\n";
            ...
            $buf .= "\t\t\$link = banco::con();\n";
            $buf .= "\t\t\$query = \"DELETE FROM " . $this->table_descriptor->getTable() . " WHERE `$pk`='\$this->$pk'\";\n";
            $buf .= "\t\tmysqli_query(\$link, \$query);\n\n";
            $buf .= "\t\treturn mysqli_affected_rows(\$link);\n";
            $buf .= "\t}\n\n";
        }
        ...
        $buf .= "\t\tmysqli_query(\$link, \$query);\n";
        $buf .= "\t\t\$this->id = mysqli_insert_id(\$link);\n";
        $buf .= "\t}\n\n";
